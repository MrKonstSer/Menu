window.addEventListener('DOMContentLoaded', () => {
    ...
    function dotsToOpacity () {
        dots.forEach(dot => dot.style.opacity = '.5');
        dots[slideIndex - 1].style.opacity = 1;
    }

    function currentAddZeroNot() {
        if(slides.length < 10) {
            current.textContent = `0${slideIndex}`;
        } else {
            current.textContent = slideIndex;
    ...
    next.addEventListener('click', () => {
        if(offset == deleteNotDigits(width) * (slides.length -1)) {  //'500px' переведем в число
            offset = 0;
        } else {
    ...
    prev.addEventListener('click', () => {
        if(offset == 0) {  //'650px' - ширина одного слайда переведем в число
            offset = deleteNotDigits(width) * (slides.length -1); //это последний слайд
        } else {
            offset -= deleteNotDigits(width);
        }
    ...
    function calcTotal() {
        if(!sex || !height || !weight || !age || !ratio) {
            result.textContent = 'Не хватает данных, жирдяй';
    ...
    function getStaticInformation(parentSelector, activeClass) {
        const elements = document.querySelectorAll(`${parentSelector} div`); //Получим все элементы 
